...
namespace gestionstock3
{
    public partial class NouvellePlanificationForm : Form
    {
        ...
        private void RemplirComboBox12()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        comboBox1.Items.Add(reader["OF"].ToString());
                    }
        ...
        {
            string selectedOF = comboBox1.SelectedItem.ToString();
            GetQuantiteAProduire(selectedOF);
        }
        private void GetQuantiteAProduire(string of)
        {
            string connectionString = "datasource=localhost;port=3306;username=root;password=;database=devnet";
            string query = "SELECT `quantite fabriquer` FROM commande WHERE OF = @OF";
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    if (reader.Read())
                    {
                        txtQuantiteAProduire.Text = reader["quantite fabriquer"].ToString();
                    }
            ...
            private void RemplirComboBox()
        {
            ...
            string connectionString = "datasource=localhost;port=3306;username=root;password=;database=devnet";
            string referenceQuery = "SELECT reference FROM articles";
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    MySqlCommand referenceCmd = new MySqlCommand(referenceQuery, connection);
                    MySqlDataReader referenceReader = referenceCmd.ExecuteReader();

                    while (referenceReader.Read())
                    {
                        txtcodearticle.Items.Add(referenceReader["reference"].ToString());
                    }
                ...
                {
                    MessageBox.Show("Une erreur est survenue lors de la récupération des références : " + ex.Message);
                }
            }
        }
        ...
        {
            {
                string datejour = dateTimePicker2.Value.ToString("yyyy-MM-dd");
                ...
                string connectionString = "datasource=localhost;port=3306;username=root;password=;database=devnet";
                string query = @"INSERT INTO productionplanification (`code_planification`,`code_article`, `qte_produire`, `production_horaire`, `Nb_jour_estimé`, `Production_journalier`, `nombre_de_bande`,`machines`,`Date_commence`,`OF`) 
        VALUES (@CodePlanification, @CodeArticle, @QuantiteAProduire, @ProductionHoraire, @NombreDeJoursEstimes, @ProductionJournaliere, @NombreDeBandes,@machines,@Date_commence,@OF)";
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@Date_commence", datejour);
                        command.Parameters.AddWithValue("@OF", comboBox1.Text);
                        ...
                        try
                        {
                            ...
                            // Enregistrement dans la table parcmachine
                            ...
                            using (MySqlCommand commandParcMachine = new MySqlCommand(queryParcMachine, connection))
                            {
                                ...
                                commandParcMachine.Parameters.AddWithValue("@Date_commence", datejour);
                                commandParcMachine.Parameters.AddWithValue("@code", comboBox2.Text);
                                ...
                                {
                                    MessageBox.Show($"Une erreur est survenue lors de l'ajout de la date de début dans la table parcmachine : {ex.Message}");
                                }
                            }
                        }
                        catch (Exception ex)
        ...
        private void txtcodearticle_SelectedIndexChanged(object sender, EventArgs e)
        {
            /* Connexion à la base de données */
            string connectionString = "datasource=localhost;port=3306;username=root;password=;database=devnet";
            string selectedReference = txtcodearticle.SelectedItem.ToString();
            string productionQuery = "SELECT productionhoraires FROM articles WHERE reference = @reference";
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    MySqlCommand productionCmd = new MySqlCommand(productionQuery, connection);
                    productionCmd.Parameters.AddWithValue("@reference", selectedReference);
                    object productionResult = productionCmd.ExecuteScalar();

                    if (productionResult != null)
                    {
                        txtProductionHoraire.Text = productionResult.ToString();
                    }
                    else
                    {
                        txtProductionHoraire.Text = "";
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Une erreur est survenue lors de la récupération des données de production horaire : " + ex.Message);
                }
            }
        }
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            if (isProductionHoraireNumeric && isNumeroDeBandeNumeric)
            {
                ...
                // Supprimer la partie décimale
                resultat = Math.Truncate(resultat);
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string connectionString = "datasource=localhost;port=3306;username=root;password=;database=devnet";
            string updateQuery = "UPDATE parcmachine SET Etat = true WHERE code = @MachineCode";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string dateCommence = dateTimePicker2.Value.ToString();
            string machineSelectionnee = comboBox2.SelectedItem.ToString();
        ...
        private void txtQuantiteAProduire_TextChanged(object sender, EventArgs e)
        {
